...
const mainColors = [
  [
    new THREE.Color(0x7676e4),
    new THREE.Color(0x45e2b6),
    "#7676e4",
    "#45e2b6",
    "#7676e41a",
    "#7676e440",
  ...
  [
    new THREE.Color(0x00539c),
    new THREE.Color(0xfc766a),
    "#00539C",
    "#FC766A",
    "#00539C1a",
    "#00539C40",
  ...
  [
    new THREE.Color(0xf93822),
    new THREE.Color(0xfdd20e),
    "#F93822",
    "#FDD20E",
    "#F938221a",
    "#F9382240",
  ...
  [
    new THREE.Color(0x6e6e6d),
    new THREE.Color(0xfad0c9),
    "#6E6E6D",
    "#FAD0C9",
    "#6E6E6D1a",
    "#6E6E6D40",
  ...
  [
    new THREE.Color(0x1d90f3),
    new THREE.Color(0xfffe03),
    "#1D90F3",
    "#FFFE03",
    "#1D90F31a",
    "#1D90F340",
  ...
  [
    new THREE.Color(0xfe7f95),
    new THREE.Color(0xffe5ee),
    "#fe7f95",
    "#ffe5ee",
    "#fe7f951a",
    "#fe7f9540",
...
const interactiveJS = () => {
  ...
  if (window.innerWidth < 1100) {
    if (aspectRatio > 3.5 / 2) {
      gsap.to(".scaleCards", {
        duration: 0,
        scale: window.innerHeight / ((1200 * 2) / 3.5),
      })
    } else {
  ...
  document.addEventListener("pointermove", (e) => {
    ...
    gsap.to(camera.rotation, {
      duration: 0.5,
      y: ((-mouse.x / 0.5) * Math.PI * 2) / 180,
      x: ((mouse.y / 0.5) * Math.PI * 2) / 180,
    })
...
const frontCanvas = () => {
  ...
  const headerAnimations = () => {
    ...
    gsap.to(".titleTopRightContainer", {
      duration: 0.75,
      delay: 3.2,
      x: 40,
      ease: "Power1.easeOut",
    })
    gsap.to(".titleBotRightContainer", {
      duration: 0.75,
      delay: 3.2,
      x: -40,
      ease: "Power1.easeOut",
    ...
    gsap.to(".topSummaryTextDiv", {
      duration: 1.25,
      delay: 3.75,
      opacity: 1,
      ease: "Power1.easeOut",
    })
    gsap.to(".bottomSummaryTextDiv", {
      duration: 1.25,
      delay: 4.25,
      opacity: 1,
      ease: "Power1.easeOut",
    ...
    gsap.to(bracketSkillsContainers[0], {
      duration: 0.2,
      delay: 5,
      x: 0,
      opacity: 1,
  ...
  const bracketSkillsContainers = document.querySelectorAll(
  ...
  for (let i = 0; i < skills.length; i++) {
    ...
    skills[i].addEventListener("mouseenter", () => {
      if (isStartupDone == true) {
        if (prevSkillIndex != i) {
          gsap.to(bracketSkillsContainers[prevSkillIndex], {
            duration: 0.2,
            x: -50,
            opacity: 0,
          })
          ...
          gsap.to(bracketSkillsContainers[i], {
            duration: 0.2,
            x: 0,
            opacity: 1,
          })

          gsap.to(skillUnderlines[prevSkillIndex], {
            duration: 0.35,
            width: 50,
            ease: "Power1.easeOut",
          })
          gsap.to(skillUnderlines[i], {
            duration: 0.35,
            width: 338,
            ease: "Power1.easeOut",
          })
  ...
  const adjectiveChange = (i) => {
    gsap.to(adjectiveTexts[i], {
      duration: adjectiveChangeDuration,
      y: -40,
      x: 10,
      opacity: 0,
      ease: adjectiveChangeEase,
    })
    gsap.to(adjectiveTexts[i], {
      duration: 0,
      delay: adjectiveChangeDuration + 0.05,
      y: 40,
      x: -10,
      opacity: 0,
    })
  ...
  document.addEventListener("pointermove", (e) => {
    ...
    for (let i = 0; i < particleGroup.length; i++) {
      const distanceFromPointerSquared =
        (particleGroup[i].position.x - pointer.x) ** 2 +
        (particleGroup[i].position.y - pointer.y) ** 2
      const directionVector = new THREE.Vector2(
        particleGroup[i].position.x - pointer.x,
        particleGroup[i].position.y - pointer.y
      )

      // Case: Affected
      if (
        distanceFromPointerSquared < parameters.radius &&
        flipValue.value < 0.5
      ) {
        // Spread
        gsap.to(particleGroup[i].position, {
          duration: 0.1,
          x: particleGroup[i].position.x + directionVector.x * 0.5,
          y: particleGroup[i].position.y + directionVector.y * 0.5,
        })

        // Size Change
        gsap.to(particleGroup[i].material, { duration: 0.1, size: 0.15 })
        gsap.to(particleGroup[i].material, {
          duration: 1,
          delay: 0.1,
          size: parameters.size,
        })
  ...
  const tick = () => {
    ...
    boxMesh.rotation.set(
      elapsedTime * boxRotations.x,
      elapsedTime * boxRotations.y,
      elapsedTime * boxRotations.z
    )
    boxBobbleGroup.position.y = Math.sin(elapsedTime) * 0.25

    tetraMesh.rotation.set(
      elapsedTime * tetraRotations.x,
      elapsedTime * tetraRotations.y,
      elapsedTime * tetraRotations.z
    )
    tetraBobbleGroup.position.y = Math.sin(elapsedTime + Math.PI / 3) * 0.25

    sphereMesh.rotation.set(
      elapsedTime * sphereRotations.x,
      elapsedTime * sphereRotations.y,
      elapsedTime * sphereRotations.z
    )
    sphereBobbleGroup.position.y =
      Math.sin(elapsedTime + (Math.PI * 2) / 3) * 0.25
...
const backCanvas = () => {
  ...
  for (let i = 0; i < mainColors.length; i++) {
    // Chain
    gltfLoader.load("./gltf/MainLinks.glb", (obj) => {
    ...
    gltfLoader.load("./gltf/SubLinks.glb", (obj) => {
      obj.scene.scale.set(chainScale, chainScale, chainScale)
    ...
    // Logo
    gltfLoader.load("./gltf/MainLogo.glb", (obj) => {
  ...
  document.querySelector(".mainPhotoSlider").addEventListener("click", () => {
    if (flipValue.value < 0.5) {
      if (isNightMode == false) {
        ...
        gsap.to(".titleTopLeftContainer", {
          duration: 0.75,
          delay: 0.1,
          x: -40,
          ease: "Power1.easeOut",
        })
        gsap.to(".titleBotLeftContainer", {
          duration: 0.75,
          delay: 0.1,
          x: 40,
          ease: "Power1.easeOut",
  ...
  const mailToEvents = (name, about, message, card, work) => {
    mailString =
      "mailto:bznz.patrickmunar@gmail.com?subject=From bznzcard.patrickmunar.com: " +
      about +
      "&Body=Hi! I am " +
      name +
      ".%0d%0a%0d%0aI just want to say " +
      message +
      "%0d%0a%0d%0a"
  ...
  document
    .querySelector(".submitButtonDiv")
    .addEventListener("mouseenter", () => {
      gsap.to(".submitButtonDiv", {
        duration: 0,
        borderWidth: "2px",
        ease: "Power1.easeOut",
      })
      if (formName != "") {
        if (formAbout != "") {
          if (formMessage != "") {
            ...
            gsap.to("textarea", {
              duration: buzzTime,
              boxShadow: "0 0 3px 3px " + mainColors[shadowIndex][2],
            })
            gsap.to("textarea", {
              duration: buzzTime,
              delay: buzzTime,
              boxShadow: "0 0 1px 0px " + mainColors[shadowIndex][2],
            })
            gsap.to("textarea", {
              duration: buzzTime,
              delay: buzzTime * 2,
  ...
  document.addEventListener("pointermove", (e) => {
    ...
    // All Links
    for (let i = 0; i < mainColors.length; i++) {
      gsap.to(linksArray[i][0].rotation, {
        duration: 1,
        y: mouse.x * Math.PI * 2,
      })
      gsap.to(linksArray[i][1].rotation, {
        duration: 1,
        y: mouse.x * Math.PI * 2,
      })
  ...
  const tick = () => {
    ...
    gsap.to(".sliderDot1", {
      duration: 0,
      scale: Math.abs(Math.sin(elapsedTime * 1)),
    })
    gsap.to(".sliderDot2", {
      duration: 0,
      scale: Math.abs(Math.sin(elapsedTime * 1 + (Math.PI * 2) / 3)),
    })
    gsap.to(".sliderDot3", {
      duration: 0,
      scale: Math.abs(Math.sin(elapsedTime * 1 + (Math.PI * 1) / 3)),
    })
  ...
  document.querySelector(".slider").oninput = () => {
    ...
    gsap.to(".businessCardBack", {
      duration: 0,
      rotateY: flipAngle - 180 + "deg",
    })

    gsap.to(chainTiltGroup.position, {
      duration: 0,
      y: -4 + (flipAngle / 180) * 4,
    })
    gsap.to(chainTiltGroup.rotation, {
      duration: 0,
      y: -Math.PI * 3 + (Math.PI * 3 * flipAngle) / 180,
    })
    gsap.to(logosSpringGroup.position, {
      duration: 0,
      y: -1 + (1 * flipAngle) / 180,
      z: -3 + (3 * flipAngle) / 180,
    })
...
const colorChange = (x) => {
  ...
  gsap.to("textarea", {
    duration: 0,
    outlineColor: mainColors[x][2],
    borderColor: mainColors[x][2],
  ...
  gsap.to(".submitBackground", {
    duration: 0,
    backgroundColor: mainColors[x][3],
  })
